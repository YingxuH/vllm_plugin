...
class MERaLiON2MultiModalProcessor(BaseMultiModalProcessor[MERaLiON2ProcessingInfo]):
    ...
    def _call_hf_processor(
        ...
        **kwargs,
        ...
        # (computed during processing, one per audio)
        if "audio_number_chunks" in results:
            chunk_sizes = results["audio_number_chunks"]
            # Remove from results as it's metadata, not a model input
    ...
    def _get_prompt_updates(
        ...
        def get_replacement_meralion2_audio(item_idx: int):
            ...
            if isinstance(number_chunk_limit, int) and number_chunk_limit > 0:
                # Keep prompt placeholders aligned with processor-side chunk truncation.
...
class MERaLiON2ForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsPP):
    ...
    def _validate_and_reshape_mm_tensor(
        ...
        Returns:
            Flattened tensor of shape [batch*seq, ...].

        Raises:
            ValueError: If input type is invalid or cannot be reshaped.
        """
        ...
        # input_features can be [B, C, 128, 3000] after batching
        # feature_attention_mask can be [B, C, 3000] after batching
        flattened_tensors: List[torch.Tensor] = []
        for tensor in tensors:
            if name == "feature_attention_mask":
                # Flatten all leading dims to chunk axis, keep time axis.
                flattened_tensors.append(tensor.reshape(-1, tensor.size(-1)))
            else:
                # Flatten all leading dims to chunk axis, keep feature/time axes.
    ...
    def _process_audio_input(
        ...
        """Process audio input through speech encoder and adapter.

        Args:
            audio_input: Dictionary containing input_features and
                feature_attention_mask.
